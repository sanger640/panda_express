...
NUM_HIST = 3
NUM_PRED = 8
PLAN_BATCH_SIZE = 5 # 1 Original + 4 Noisy trajectories
NOISE_STD = 5e-3
NUM_TEST_SAMPLES = 25
...
def main():
    ...
    with env.begin() as txn:
        ...
        for s_idx in range(NUM_TEST_SAMPLES):
            ...
            cluster_indices = resp.get('max_patch_idx', None) # Expected shape: (B-1, 5)
            ...
            for t in range(NUM_PRED):
                ...
                for b in range(1, PLAN_BATCH_SIZE):
                    ...
                    # Highlight the 5-patch cross on the final prediction frame
                    if t == NUM_PRED - 1 and cluster_indices is not None:
                        # B-1 because server returns LE for noisy batch indices
                        batch_cluster = cluster_indices[b-1] 
                        
                        # Threshold Logic (BGR: Green if stable, Red if divergent)
                        rect_color = (0, 255, 0) if le_val < 1.0 else (0, 0, 255)
                        
                        for p_idx in batch_cluster:
                            y, x = (p_idx // 14) * 16, (p_idx % 14) * 16
                            cv2.rectangle(noisy_p, (x, y), (x+16, y+16), rect_color, 1)
                    
                    col_frames.append(add_label(noisy_p, f"NOISY {b} (LE:{le_val:.2f})", (0,255,255)))
